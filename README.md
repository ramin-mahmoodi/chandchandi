...
سامانه دریافت نرخ‌ها به گونه‌ای مهندسی شده است که فرآیند اعتبارسنجی کلاینت، توزیع درخواست در شبکه و تاب‌آوری در برابر اختلالات اینترنتی را در چهار فاز مجزا تضمین کند:

```mermaid
flowchart TD
    subgraph Phase1 ["فاز اول: گردآوری و آماده‌سازی پارامترها"]
        A["شروع چرخه استعلام (Auto / Manual)"] --> B["مقداردهی پارامترهای پایه کلاینت\n(platform: android, v: 3.1.0, key, ts, slug, type)"]
    end

    subgraph Phase2 ["فاز دوم: خط لوله رمزنگاری و امضای دیجیتال"]
        B --> C["مرتب‌سازی الفبایی کلیدها (Lexicographical Key Sorting)"]
        C --> D["تولید رشته معیار کدگذاری‌شده (Canonical Query String)"]
        D --> E["تولید امضا با کلید محرمانه\nHMAC-SHA256(SecretKey, CanonicalQuery)"]
        E --> F["افزودن امضا به کوئری و تولید نشانی نهایی (&sign=...)"]
    end

    subgraph Phase3 ["فاز سوم: لایه شبکه و استراتژی چندمسیره (Multi-Route)"]
        F --> G{"انتخاب مسیر ارسال درخواست"}
        G -->|"مسیر اول (مستقیم)"| H["ارسال مستقیم به سرور اصلی\napi.alanchand.com"]
        G -->|"مسیر دوم (شخصی)"| I["پروکسی اختصاصی کاربر\nCloudflare Worker"]
        G -->|"مسیر سوم (عمومی)"| J["زنجیره پروکسی‌های پشتیبان\nAllOrigins / CodeTabs"]
    end

    subgraph Phase4 ["فاز چهارم: ارزیابی پاسخ، ذخیره‌سازی و تاب‌آوری"]
        H -->|"پاسخ موفق (HTTP 200)"| K["اعتبارسنجی ساختار JSON و وضعیت success"]
        I -->|"پاسخ موفق (HTTP 200)"| K
        J -->|"پاسخ موفق (HTTP 200)"| K
        
        K --> L["به‌روزرسانی کش محلی در مرورگر\n(LocalStorage با Timestamp)"]
        L --> M["رندر بلادرنگ در کارت‌ها، تیکر و ماشین‌حساب"]
        
        H -->|"خطای شبکه یا اختلال"| N{"سلسله‌مراتب بازیابی خطا (Fallback Hierarchy)"}
        I -->|"خطای شبکه یا اختلال"| N
        J -->|"خطای شبکه یا اختلال"| N
        
        N -->|"سطح اول"| O["بارگذاری آخرین وضعیت پایدار از LocalStorage"]
        N -->|"سطح دوم (در غیاب کش)"| P["بارگذاری دیتاست آفلاین همراه (Bundled Fallback Data)"]
    ...
    classDef highlight fill:#b6ace4,stroke:#000000,stroke-width:2px,color:#000000;
    classDef success fill:#97ee88,stroke:#000000,stroke-width:2px,color:#000000;
    class M highlight;
    class K success;
```

#### دیاگرام ساختاریافته جریان داده و وضعیت‌ها (Dataflow Architecture)

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│ ۱. مقداردهی اولیه پارامترها (Client Parameters Initialization)               │
│    key: fc6cdb49... | platform: android | v: 3.1.0 | ts: Unix | slug: [...] │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ ۲. مرتب‌سازی الفبایی و تولید رشته معیار (Canonical Message Construction)     │
│    key=... & platform=android & slug=... & ts=... & type=full & v=3.1.0     │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ ۳. امضای رمزنگاری‌شده (HMAC-SHA256 Digital Signing)                         │
│    sign = HMAC_SHA256(SecretKey, CanonicalMessage) -> 64-char Hex Hash      │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ ۴. ساخت نشانی نهایی استعلام (Signed URL Generation)                         │
│    https://api.alanchand.com/app/v2.php?...&sign=<hash>                     │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │
                                       ▼
                              [درخواست در شبکه]
            ...
            ┌──────────────────────────┴──────────────────────────┐
            │                                                     │
     (پاسخ موفق ۲۰۰)                                      (خطا یا قطعی شبکه)
            │                                                     │
            ▼                                                     ▼
┌───────────────────────────┐                         ┌───────────────────────┐
│ پارس داده‌های JSON و ثبت  │                         │ ۵. بازیابی از کش      │
│ در LocalStorage مرورگر    │                         │    (LocalStorage)     │
└───────────┬───────────────┘                         └───────────┬───────────┘
            │                                                     │
            │                                           (در صورت نبود کش قبلی)
            │                                                     │
            ▼                                                     ▼
┌───────────────────────────┐                         ┌───────────────────────┐
│ به‌روزرسانی کارت‌های قیمت │◄────────────────────────┤ ۶. بارگذاری دیتاست    │
│ تیکر و ماشین‌حساب تبدیل   │                         │    آفلاین پشتیبان     │
└───────────────────────────┘                         └───────────────────────┘
```
