...
### دیاگرام متنی جریان استعلام و معماری بازیابی
...
+-------------------------------------------------------------+
| ۱. گردآوری پارامترها:                                       |
|    key, platform, v, type, ts (Unix Timestamp), slug        |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
| ۲. مرتب‌سازی الفبایی کلیدها و ساخت رشته معیار (Canonical)     |
|    key=...&platform=...&slug=...&ts=...&type=...&v=...      |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
| ۳. محاسبه امضای رمزنگاری‌شده (HMAC-SHA256 + Secret Key)      |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
| ۴. تولید URL کامل و معتبر همراه با sign                     |
+-------------------------------------------------------------+
                               |
                               v
             [ارسال درخواست GET به سرور API]
                               |
            +------------------+------------------+
            |                                     |
     (پاسخ ۲۰۰ معتبر)                     (خطای شبکه / اختلال)
            |                                     |
            v                                     v
+------------------------+             +------------------------+
| پارس داده‌های JSON     |             | ۵. بازیابی از Cache    |
| و ذخیره در LocalStorage|             |    (LocalStorage مرورگر|
+------------------------+             +------------------------+
                                                  |
                                         (در صورت عدم وجود Cache)
                                                  |
                                                  v
                                       +------------------------+
                                       | ۶. استفاده از دیتاست   |
